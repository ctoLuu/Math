...
class GA(object):
    ...
    def rand_chrom(self):
        for i in range(self.size_pop):
            ...
            for j in range(self.num):
                random_number = np.random.uniform(160, 201.5)
    ...
    def comp_fit(self, CN):
        ...
        res = fixed - 0.003 * 48 * max / 1.05 + (charge + 27) * 0.9442
        for i in range(floor(max / 1.05), floor(max / 1.05) - 100, -1):
            current = fixed - 0.003 * 48 * i * 1.05 - 0.003 * 96 * (max - i * 1.05) + (charge + 27) * 0.9442
            if current > res:
                res = current
        return res
    ...
    def info(self, CN):
        ...
        for i in range(floor(max / 1.05), floor(max / 1.05) - 100, -1):
            ...
            if current > res:
                res = current
                flag = i
        demand_price = 48 * flag * 1.05 + 96 * (max - flag * 1.05)
        charge_price = (charge+27) * 0.9442 + fixed
        return res, demand_price, charge_price

if __name__ == "__main__":
    df = pd.read_excel('./预测结果_二并柜_2023-08-29.xlsx')
