...
class GA(object):
    ...
    def rand_chrom(self):
        for i in range(self.size_pop):
            ...
            for j in range(self.num):
                random_number = np.random.uniform(80, 101.4)
    ...
    def comp_fit(self, CN):
        ...
        res = fixed - 0.003 * 48 * max / 1.05 + charge * 0.8257
        for i in range(floor(max / 1.05), floor(max / 1.05) - 100, -1):
            current = fixed - 0.003 * 48 * i * 1.05 - 0.003 * 96 * (max - i * 1.05) + charge * 0.8257
            if current > res:
                res = current
        return res
    ...
    def info(self, CN):
        ...
        charge_price = charge * 0.8257 + fixed
