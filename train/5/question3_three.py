...
class GA(object):
    ...
    def comp_fit(self, CN):
        ...
        res = fixed - 0.003 * 48 * max / 1.05 + (charge + 28.8) * 0.9442
        for i in range(floor(max / 1.05), floor(max / 1.05) - 100, -1):
            current = fixed - 0.003 * 48 * i * 1.05 - 0.003 * 96 * (max - i * 1.05) + (charge + 28.8) * 0.9442
            if current > res:
                res = current
        return res
    ...
    def info(self, CN):
        ...
        charge_price = fixed + (charge + 28.8) * 0.9442
