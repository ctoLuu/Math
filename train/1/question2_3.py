...
true_data = pd.read_excel('./handledData2.xlsx')  # 填你自己的数据地址

target = 'OT'

# 这里加一些数据的预处理, 最后需要的格式是pd.series

true_data = np.array(true_data['CO(GT)'])
...
if Train:
    ...
    plt.plot(results, label='forecast', color='red', linestyle='--', linewidth=0.1)  # 预测值
