...
class ICBS_Solver(object):
    ...
    def print_results(self, node):
        ...
        print("Solution:")
        # 读取 Excel 文件中的指定工作表
        df = pd.read_excel('./results_第二问.xlsx', sheet_name='16x16map')
        df['位置列表'] = df['位置列表'].astype('object')
        ...
        # 计算时间开销
        time = [len(node['paths'][i]) - 1 for i in range(len(node['paths']))]
        df['位置列表'] = node['paths']
        df['时间开销'] = time
        ...
        # Excel 文件名和工作表名
        filename = './results_第二问.xlsx'
        sheet_name = '16x16map'

        # 加载现有的 Excel 文件
        book = load_workbook(filename)

        # 使用 openpyxl 引擎并指定附加模式 ('a')
        with pd.ExcelWriter(filename, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
            # 将 DataFrame 写入指定工作表，覆盖旧内容
