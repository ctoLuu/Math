...
def getCount(count):
    ...
    for sub_count in Count:
        ...
        for i in range(7):
            for j in range(15):
                if j == 5:
                    sub_count[i].append(count[j] * (1 + wheat_growth_rates[flag_wheat]) * (1 + beta * 0.05))
                    flag_wheat += 1
                elif j == 6:
                    sub_count[i].append(count[j] * (1 + corn_growth_rates[flag_corn]) * (1 + beta * 0.05))
                    flag_corn += 1
                else:
            ...
            for i in range(7):
                ...
                for j in range(15):
                    if column_sums[j] > self.count[epoch][i][j]:
                        ...
                        fitness += self.count[epoch][i][j] * price[j] - cost_sums[j]
                    else:
                        leave.append(j)
                    ...
                    for o_index, o in enumerate(over):
                        if (l < 5 and o < 5) or (l >= 5 and o >= 5):
                            ...
                            if transfer_amount > 0:
                                ...
                                fitness += transfer_amount * price[o]

                for o_index, o in enumerate(over):
                    if over_count[o_index] > 0:
                        fitness += over_count[o_index] * price[o] / 2
