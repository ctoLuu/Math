...
def getCount(count):
    ...
    for sub_count in Count:
        for i in range(7):
            for j in range(26):  # Adjusted to cover the full range
                if 1 <= j and j < 22:
                    sub_count[i].append(origin_count[j] * (1 + sales_growth_rates[flag]) * (1 + beta * 0.05) * (1 + gamma * 0.05))
                    flag += 1
                elif j < 26:
                    sub_count[i].append(origin_count[j] * (1 + sales_growth_rates[flag]) * (1 + beta * 0.05))
                    flag += 1
                else:
                    sub_count[i].append(origin_count[j] * (1 + sales_growth_rates[flag]) * (1 + beta * 0.05))
                    flag += 1
...
class GA(object):
    ...
    def get_chrom(self, rand_ch):
        for index, seed in enumerate(rand_ch):
            try:
                if index < 24:
                    for j in range(0, 5, 1):  # Iterate every three rows
            ...
            for i in range(7):
                ...
                for j in range(19):
                    if column_sums[j] > self.count[epoch][i][j]:
                        if j != 0:
                            over.append(j)
                            over_count.append(column_sums[j] - self.count[epoch][i][j])
                        fitness_1 += self.count[epoch][i][j] * self.price[epoch][i][j] - cost_sums[j]
                    else:
                        if j != 0:
                            leave.append(j)
                            leave_count.append(self.count[epoch][i][j] - column_sums[j])
                        fitness_1 += column_sums[j] * self.price[epoch][i][j] - cost_sums[j]

                for l_index, l in enumerate(leave):
                    for o_index, o in enumerate(over):
                        if (l < 4 and o < 4) or (l >= 4 and o >= 4):
                            transfer_amount = min(over_count[o_index], leave_count[l_index])
                            if transfer_amount > 0:
                                over_count[o_index] -= transfer_amount
                                leave_count[l_index] -= transfer_amount

                                fitness_1 += transfer_amount * self.price[epoch][i][o]

                for o_index, o in enumerate(over):
                    if over_count[o_index] > 0:
                        fitness_1 += over_count[o_index] * self.price[epoch][i][o] / 2
                ...
                for j in range(1, 26):
                    if j < 19:
                        if column_sums[j] > self.count2[epoch][i][j]:
                            if j != 0:
                                over.append(j)
                                over_count.append(column_sums[j] - self.count2[epoch][i][j])
                            fitness_2 += self.count2[epoch][i][j] * self.price[epoch][i][j + 44 - 1] - cost_sums[j]
                        else:
                            if j != 0:
                                leave.append(j)
                                leave_count.append(self.count[epoch][i][j] - column_sums[j])
                            fitness_2 += column_sums[j] * self.price[epoch][i][j + 44 - 1] - cost_sums[j]
                    else:
                        if column_sums[j] > self.count2[epoch][i][j]:
                            over2.append(j)
                            over_count2.append(column_sums[j] - self.count2[epoch][i][j])
                            fitness_2 += self.count2[epoch][i][j] * self.price[epoch][i][j + 37 - 1] - cost_sums[j]
                        else:
                            leave2.append(j)
                            leave_count2.append(self.count[epoch][i][j] - column_sums[j])
                            fitness_2 += column_sums[j] * self.price[epoch][i][j + 37 - 1] - cost_sums[j]
                    ...
                    for o_index, o in enumerate(over):
                        if (l < 4 and o < 4) or (l >= 4 and o >= 4):
                            ...
                            if transfer_amount > 0:
                                ...
                                fitness_2 += transfer_amount * self.price[epoch][i][o + 44 - 1]
                for o_index, o in enumerate(over):
                    if over_count[o_index] > 0:
                        fitness_2 += over_count[o_index] * self.price[epoch][i][o + 44 - 1] / 2

                for l_index, l in enumerate(leave2):
                    for o_index, o in enumerate(over2):
                        if (l < 22 and o < 22) or (l >= 22 and o >= 22):
                            transfer_amount = min(over_count2[o_index], leave_count2[l_index])
                            if transfer_amount > 0:
                                over_count2[o_index] -= transfer_amount
                                leave_count2[l_index] -= transfer_amount

                                fitness_2 += transfer_amount * self.price[epoch][i][o + 37 - 1]
                for o_index, o in enumerate(over2):
                    if over_count2[o_index] > 0:
                        fitness_2 += over_count2[o_index] * self.price[epoch][i][o + 37 - 1] / 2
